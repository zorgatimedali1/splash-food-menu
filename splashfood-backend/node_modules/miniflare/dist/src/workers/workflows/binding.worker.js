...
function registerRollbackFn(registry, registration) {
  let { cacheKey, fn, stepName, output, config } = registration, existing = registry.get(cacheKey);
...
async function executeRollbacks(engine, triggerError) {
  ...
  for (let [i, [cacheKey, entry]] of entries.entries()) {
    let ctx = engine.createRollbackContext({ cacheKey });
    try {
...
var defaultConfig = {
  ...
  #registerRollback(options) {
    let { cacheKey, rollbackFn, stepName, output, rollbackConfig } = options;
