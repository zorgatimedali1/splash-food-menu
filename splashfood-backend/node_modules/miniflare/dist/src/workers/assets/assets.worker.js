...
var AssetWorkerOuter = class extends WorkerEntrypoint {
  ...
  async fetch(request) {
    ...
    try {
      ...
      return analytics.setData({ status: response.status }), response.status >= 500 && analytics.setData({ error: "inner entrypoint error" }), response;
    } catch (err) {
  ...
  async fetch(request) {
    this.env.JAEGER ??= mockJaegerBinding();
    let traceContext = this.ctx.props?.traceContext ?? null, cohort = this.ctx.version?.cohort, response = await this.env.JAEGER.runWithSpanContext(
      traceContext,
      () => runFetchRequest(
        request,
        this.env,
