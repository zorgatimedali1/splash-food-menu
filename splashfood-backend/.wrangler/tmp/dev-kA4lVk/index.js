...
// src/utils/jwt.ts
var base64url = {
  encode: /* @__PURE__ */ __name((buf) => btoa(String.fromCharCode(...new Uint8Array(buf))).replace(/\+/g, "-").replace(/\//g, "_").replace(/=+$/, ""), "encode"),
...
var handleAuth = /* @__PURE__ */ __name(async (request, env2, path) => {
  ...
  if (path === "/api/auth/register" && method === "POST") {
    ...
    try {
      ...
      if (count3 > 0) return jsonResponse(errorResponse("Registration is locked. Admin already exists."), 403);
...
var handleCategories = /* @__PURE__ */ __name(async (request, env2, path, pathParts) => {
  ...
  if (pathParts.length === 3 && method === "PUT") {
    ...
    if (contentType.includes("multipart/form-data")) {
      const formData = await request.formData();
      if (formData.get("name")) fields.name = formData.get("name").toUpperCase();
      if (formData.get("description") !== null) fields.description = formData.get("description");
      if (formData.get("sort_order")) fields.sort_order = parseInt(formData.get("sort_order"), 10);
      const imageFile = formData.get("image");
      if (imageFile && imageFile.size > 0) {
        const variants = await processAndUploadImage(env2, imageFile, "categories");
      ...
      const body = await request.json();
      if (body.name) fields.name = body.name.toUpperCase();
      if (body.description !== void 0) fields.description = body.description;
      if (body.sort_order !== void 0) fields.sort_order = body.sort_order;
      if (body.is_active !== void 0) fields.is_active = body.is_active;
    }
...
var handleProducts = /* @__PURE__ */ __name(async (request, env2, path, pathParts) => {
  ...
  if (pathParts.length === 3 && method === "PUT") {
    ...
    if (contentType.includes("multipart/form-data")) {
      ...
      if (formData.get("description") !== null) fields.description = formData.get("description");
      if (formData.get("price")) fields.price = parseFloat(formData.get("price"));
      if (formData.get("category_id")) fields.category_id = parseInt(formData.get("category_id"), 10);
      if (formData.get("sort_order")) fields.sort_order = parseInt(formData.get("sort_order"), 10);
      if (formData.get("is_active") !== null) fields.is_active = parseInt(formData.get("is_active"), 10);
      const imageFile = formData.get("image");
      if (imageFile && imageFile.size > 0) {
        const variants = await processAndUploadImage(env2, imageFile, "products");
        fields.image_url = variants.web;
      ...
      allowed.forEach((k) => {
        if (body[k] !== void 0) fields[k] = body[k];
      });
    }
...
var handleSupplements = /* @__PURE__ */ __name(async (request, env2, path, pathParts) => {
  ...
  if (path === "/api/supplements" && method === "POST") {
    ...
    if (!name || price === void 0) return jsonResponse(errorResponse("Name and price are required"), 400);
    try {
      const result = await env2.DB.prepare(
  ...
  if (pathParts.length === 3 && method === "PUT") {
    ...
    const body = await request.json();
    const fields = {};
    if (body.name !== void 0) fields.name = body.name;
    if (body.price !== void 0) fields.price = body.price;
    if (body.is_active !== void 0) fields.is_active = body.is_active;
    if (Object.keys(fields).length === 0) return jsonResponse(errorResponse("No fields to update"), 400);
    const setClause = Object.keys(fields).map((k) => `${k} = ?`).join(", ");
...
// .wrangler/tmp/bundle-jhZu1W/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-jhZu1W/middleware-loader.entry.ts
